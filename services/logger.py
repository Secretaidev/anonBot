...
async def log_to_channel(
    ...
    *,
    ...
    if session_id:
        lines.append(f"<b>🔑 Session</b>  <code>{escape(session_id)}</code>")

    if partner:
        p_name = escape(display_name(partner))
        p_user = f"@{escape(partner.username)}" if partner.username else "—"
        lines.extend(
            [
                "━━━━━━━━━━━━━━━━━━━━",
    ...
    if message_type and content is not None:
        preview = escape(content[:400])
        if len(content) > 400:
            preview += "…"
        lines.extend(
            [
                "━━━━━━━━━━━━━━━━━━━━",
                f"<b>💬 Type</b>  {escape(message_type)}",
                f"<b>📝 Content</b>\n{preview}",
            ]
        )

    if extra:
        lines.extend(["━━━━━━━━━━━━━━━━━━━━", escape(extra)])
